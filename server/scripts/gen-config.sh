...
#   --dial HOST         Address written into client profiles (default: --host).
#                       Use an IPv6 literal so clients QUIC to AAAA (v1.5.6).
...
#   --pool-v6 CIDR      IPv6 client pool (default: fd00:8::/64). Empty to disable.
#   --tun-addr-v6 CIDR  Server IPv6 TUN address (default: fd00:8::1/64).
#   --route-v6 CIDR     IPv6 route advertised (default: ::/0).
...
# Client profile host:port. IPv6 literals need brackets.
client_addr() {
  local h="${DIAL_HOST:-$HOST}"
  if is_ip6 "$h"; then
    printf '[%s]:%s' "$h" "$PORT"
  else
    printf '%s:%s' "$h" "$PORT"
...
if is_ip "$HOST" || is_ip6 "$HOST"; then
  SAN="IP:$HOST,$SAN"
else
  SAN="DNS:$HOST,$SAN"
fi
...
if [ -n "$DIAL_HOST" ] && [ "$DIAL_HOST" != "$HOST" ] && [ "$DIAL_HOST" != "$EXTRA_IP" ]; then
  if is_ip "$DIAL_HOST" || is_ip6 "$DIAL_HOST"; then
    SAN="$SAN,IP:$DIAL_HOST"
  fi
fi
